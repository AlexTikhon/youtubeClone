...
Next.js web --REST + HTTP-only cookie--> NestJS API --> PostgreSQL
     ...
     +----------- signed PUT ------------> MinIO
                                            ^
API -- PostgreSQL outbox --> publisher --> Redis/BullMQ --> worker -- ffprobe/FFmpeg
                                            |
                                            +--> MinIO thumbnail + HLS
```
...
for users, sessions, video state, upload intents, asset metadata, and the purpose-specific processing
outbox. Redis carries disposable queue
...
## Recoverable processing boundary

```text
upload completion / FAILED retry
             |
             v
short PostgreSQL transaction
  +-- state + processingGeneration
  +-- ProcessingOutbox(videoId, generation)
             |
          COMMIT
             v
API publisher -- deterministic BullMQ job ID --> worker
```

Generation 1 is the initial submission; an owner retry increments the generation. Outbox publication
can be repeated after Redis failure or API restart. Every worker boundary checks that its job's
generation still equals the video row, and generated bytes are isolated under the generation prefix.
Only the successful asset transaction makes a generation authoritative. This resolves both the
PostgreSQL/Redis dual-write gap and delayed old-job races without introducing a message broker or a
...
The guarded URL is stable even when visibility changes. PUBLIC responses therefore use
`public, max-age=0, must-revalidate`; UNLISTED and PRIVATE responses use `private, no-store`. The
underlying generated bytes are immutable, but authorization policy is not, so a shared cache must
revalidate before serving a formerly public URL. At CDN scale this would be replaced by versioned or
signed delivery plus explicit invalidation or authorization tokens.

The storage adapter classifies a genuine missing key separately from dependency failure. Media reads
map those outcomes to safe 404 `MEDIA_NOT_FOUND` and 503 `MEDIA_STORAGE_UNAVAILABLE` responses;
upload completion and processing retry likewise preserve their domain missing-object conflict while
reporting storage downtime as 503. Bucket names, keys, endpoints, and SDK diagnostics remain internal.
...
seconds while an owned video is UPLOADED or PROCESSING and stops at READY or FAILED. FAILED Studio
rows show a safe reason, generation, update time, and an owner retry mutation. XHR is isolated behind
`shared/upload/upload-file.ts`, exposing
...
Failure leaves `DELETING` for an owner retry. S3 multi-object responses are checked for per-key
`Errors`; a partial cleanup is failure and cannot be followed by database deletion. If a worker
uploads after deletion begins, its final state claim fails, its asset transaction rolls back, and it
removes generated prefixes. Worker best-effort cleanup also recognizes and logs partial deletion.
This prevents both database resurrection and silently accepted normal-operation storage leaks.

## Operational health and logs

The API exposes process liveness and PostgreSQL/Redis readiness. The worker's separate port exposes
process liveness and readiness across PostgreSQL, Redis/BullMQ, MinIO buckets, FFmpeg, and ffprobe.
...
guarded thumbnails and VOD HLS masters/variants/segments must revalidate authorization on every use;
non-public media, including UNLISTED media, is `private, no-store`.
